...
let render_mcl surface positions =
  let minimum =
    Array.fold_left (fun x (s, _) -> min s x) infinity positions
  and maximum =
    Array.fold_left (fun x (s, _) -> max s x) neg_infinity positions in
  ...
  Array.iter
    (fun (s, p) -> draw_position surface (get_color s) p 7)
    positions
;;
